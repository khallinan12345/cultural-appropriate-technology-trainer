...
const CulturalTrainerLocal = () => {
  ...
  const callLlamaAPI = async (prompt, role) => {
    ...
    try {
      // Use the correct endpoint - just /api/generate, no full URL
      const endpoint = '/api/generate';
      
      console.log('Calling endpoint:', endpoint);
      console.log('Request URL:', window.location.origin + endpoint);
  ...
  const generateProgressEvaluation = async () => {
    console.log('=== Starting generateProgressEvaluation ===');
    setIsLoading(true);
    
    // Collect all feedback from the current session
    const currentSessionFeedback = advisorMessages.map(msg => msg.content).join('\n\n');
    console.log('Current session feedback:', currentSessionFeedback);
    
    // Create a prompt for the AI
    const prompt = `
  You are an AI Cultural Training Progress Evaluator. Generate a comprehensive progress evaluation based on the following feedback from a cultural training session focused on US business communication.
  ...
  Generate a progress evaluation with the following sections:
  
  # Cultural Training Progress Evaluation
  
  ## Session Information
  - Date: ${new Date().toLocaleDateString()}
  - Session Focus: US Business Communication
  
  ## Previous Status Summary
  ${trainingProgress ? 'Summarize key points from previous training' : 'First training session - no previous history'}
  
  ## Current Session Analysis
  
  ### Strengths Demonstrated
  - List specific strengths shown in this session
  - Reference specific examples from the feedback
  ...
  Compare current performance to previous sessions (if applicable)
  
  ## Recommendations for Next Session
  - Specific areas to focus on
  - Suggested practice scenarios
  - Communication techniques to develop
  
  ## Key Takeaways
  - 2-3 main points from this session
  ...
  `;
    ...
    try {
      console.log('Calling API for progress evaluation...');
      const evaluation = await callLlamaAPI(prompt, role);
      console.log('Progress evaluation received:', evaluation);
      setIsLoading(false);
      return evaluation;
    } catch (error) {
      setIsLoading(false);
...
const endSession = async () => {
  ...
  console.log('Setting training progress to:', updatedProgress);
  ...
  const formatMarkdown = (markdown) => {
    console.log('=== formatMarkdown input ===', markdown);
    
    if (!markdown) {
      console.log('No markdown provided');
      return '<p>No training progress available yet.</p>';
    }
    
    // Convert markdown to HTML with better formatting
    let html = markdown
      // Headers - use multiline mode flag
      .replace(/^### (.*?)$/gm, '<h3>$1</h3>')
      .replace(/^## (.*?)$/gm, '<h2>$1</h2>')
      .replace(/^# (.*?)$/gm, '<h1>$1</h1>')
      // Bold
      .replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>')
      // Lists with better matching
      .replace(/^\* (.*?)$/gm, '<li>$1</li>')
      .replace(/^- (.*?)$/gm, '<li>$1</li>')
    ...
    console.log('=== formatMarkdown output ===', html);
    return html;
  };
      ...
      {showServerConfig ? (
        <div className="server-config-container">
          ...
          <div className="server-setup-instructions">
            ...
            <p>This application uses OpenAI's GPT models for natural language processing.</p>
            <p>Your Express server proxies the requests to OpenAI's API.</p>
          </div>
        </div>
      ) : (
        <>
          ...
          <div className="chat-container">
            ...
            <div className="assistants-container">
              ...
              <div className="progress-window">
                ...
                >
                  {isLoading ? (
                    <div className="loading-indicator">Generating progress evaluation...</div>
                  ) : trainingProgress && trainingProgress !== 'Cultural Training Progress' ? (
                    <div 
                    ...
                    <div className="empty-progress">
                      <p>No training progress available yet.</p>
                      <p>Click "End Session" after completing some interactions to generate your progress evaluation.</p>
                    </div>
                  )}
                </div>
              </div>
